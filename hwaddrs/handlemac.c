...
static int readAddr(const struct misc_entry *const entry, macaddr_t *buf)
{
	...
	if(pipe(comm)<0)
		...
		if(dup2(comm[1], 1)<0) {
			__android_log_print(ANDROID_LOG_ERROR, TAG, errfmt,
"dup2", strerror(errno));
			exit(1);
		}
		...
		** writing to these, errors are irrelevant */
		...
		__android_log_print(ANDROID_LOG_ERROR, TAG, errfmt, "execve",
strerror(errno));
...
"child process failed to exit; retrieving true MAC address failed");
	...
	__android_log_print(ANDROID_LOG_ERROR, TAG, errfmt, errmsg,
strerror(errno));
