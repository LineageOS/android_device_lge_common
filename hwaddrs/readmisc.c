...
int main(int argc, char **argv)
{
	...
	retfd=strtol(argv[1], &end, 16);
	if(!argv[1]||*end||retfd<=2) {
		errmsg="Return file descriptor number invalid";
		goto fail;
	}
	...
	if(write(retfd, buf.macaddr, sizeof(macaddr_t))!=sizeof(macaddr_t)) {
